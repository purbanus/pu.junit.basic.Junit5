...
/**
 * Dit komt uit https://www.baeldung.com/java-junit-hamcrest-guide
 */
//====================================================================================================================
// BELANGRIJK
...
Hamcrest is commonly used with junit and other testing frameworks for making assertions. Specifically, instead of using junit‘s numerous 
assert methods, we only use the API’s single assertThat statement with appropriate matchers.
...
freestar
Werkt ook als Animal een class is.
*/
@Test
...
We can check if the Person bean has the property, name like so:
*/
@Test
...
Let’s pay close attention to this matcher, closeTo. The first argument, the operand, is the one to which the target is compared and 
the second argument is the allowable deviation from the operand . This means that if the target is operand+deviation or 
operand-deviation, then the test will pass.
*/
@Test
...
Assertion on Strings is made easier, neater and more intuitive with Hamcrest‘s text matchers. We are going to take a 
...
*/
@Test
 public void given2Strings_whenEqual_thenCorrect_Die_was_er_boven_al() {
    String a = "foo";
    String b = "FOO";
    assertThat(a, equalToIgnoringCase(b));
}
...
We can define our own matcher by extending TypeSafeMatcher. In this section, we will create a custom matcher which allows 
a test to pass only when the target is a positive integer.
Zie IsPositiveInteger.class
